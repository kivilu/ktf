...
public class ErrorLoader {
    ...
    public void init() {
        ...
        if (errorCodeDb != null)
            ErrorKit.me().updateErrorCodeMap(errorCodeDb);

        // 2.将属性文件中的错误码信息同步到数据库
        syncErrorCode(ErrorKit.me().getErrorCodeMap(), errorCodeDb);

        Timer time = new Timer();
        time.schedule(new ErrorCodeLoadTask(), 60 * 1000, 60 * 1000);
    }

    public static ErrorLoader me() {
        ErrorLoader bean = SpringContextHolder.getBeanNoAssert(ErrorLoader.class);
        return bean;
    }
    ...
    private void syncErrorCode( Map<String, String[]> errorCodeProp, Map<String, String[]> errorCodeDb ) {
        if (errorCodeDb == null || errorCodeProp == null)
            return;
    ...
    private class ErrorCodeLoadTask extends TimerTask {

        @Override
        public void run() {
            if (needLoad.compareAndSet(true, false))
                loadErrorCode();
        }
